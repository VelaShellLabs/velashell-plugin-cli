...
internal static partial class Program
{
    ...
    private static int Doctor(string[] args)
    {
        ...
        // 拆库(2026-08-27)之后本工具的版本与契约 SDK 的版本是**两条线**,不能再拿
        // VelaPluginApi.SdkVersion 冒充工具版本 —— 那会让 `doctor` 报一个与
        // `vela-plugin --version` 对不上的数字,而这一行的全部用处就是让人对版本。
        // 所以两个都打:工具自己的包版本,以及它编译时绑定的那版契约。
        Console.WriteLine($"vela-plugin {ToolVersion} (sdk {VelaPluginApi.SdkVersion}, apiLevel {VelaPluginApi.Level})");
